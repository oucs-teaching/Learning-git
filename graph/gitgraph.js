(function (global, factory) {
	...
	/**
	 * Provide an appropriately contrasting shade of grey.
	 * Expects parameters as hex strings (e.g. "#ffffff")
	 * @param colour
	 * @param defaultColour
	 */
	function contrastingGrey(colour, defaultColour) {
	    if (colour.length != 7) return defaultValue;
	    if (colour.charAt(0) != "#") return defaultValue;
	    var R = parseInt(colour.substring(1, 3), 16);
	    var G = parseInt(colour.substring(3, 5), 16);
	    var B = parseInt(colour.substring(5, 7), 16);
	    var bright = Math.sqrt((R * R * 0.241) + (G * G * 0.691) + (B * B * 0.068));
	    if (bright < 64) return "#c0c0c0";
	    if (bright < 128) return "#e7e7e7";
	    if (bright < 192) return "#636363";
	    return "#898989";
	}
	exports.contrastingGrey = contrastingGrey;
	/**
	...
	function arrowSvgPath(graph, parent, commit) {
	    ...
	    var h = commitRadius + graph.template.arrow.offset;
	    if (graph.template.arrow.centered) {
           const deltaX = (parent.x - commit.x) / 2.0;
           const deltaY = (parent.y - commit.y) / 2.0;
           h = Math.sqrt((deltaX * deltaX) + (deltaY * deltaY)) - (size / 2);
   	 }
	    // Delta between left & right (radian)
	    const delta = graph.template.arrow.angle;
	    // Alpha angle between parent & commit (radian)
	    const alpha = getAlpha(graph, parent, commit);
	    // Top
	    var x1 = h * Math.cos(alpha);
	    var y1 = h * Math.sin(alpha);
	    if ((graph.template.arrow.centered) &&
	        (commit.branches !== parent.branches) &&
           (commit.branches[0] !== parent.branches[0])) {
           const deltaX = Math.abs(commit.x - parent.x);
           const deltaY = Math.abs(commit.y - parent.y);
           if (parent.branches.includes(commit.branches[0])) { //diverging
               if (graph.reverseArrow) {
                   if (graph.isVertical) {
                       if (deltaY > graph.template.commit.spacing)
                           x1 += deltaX;
                   } else {
                       if (deltaX > graph.template.commit.spacing)
                           y1 += deltaY;
                   }
               }
           } else { //merging
	    ...
	    // Bottom right
	    const x2 = x1 + size * Math.cos(alpha - delta);
	    const y2 = y1 + size * Math.sin(alpha - delta);
	    // Bottom center
	    const x3 = x1 + (size / 2) * Math.cos(alpha);
	    const y3 = y1 + (size / 2) * Math.sin(alpha);
	    // Bottom left
	    const x4 = x1 + size * Math.cos(alpha + delta);
	    const y4 = y1 + size * Math.sin(alpha + delta);
	...
	function getAlpha(graph, parent, commit) {
       ...
       if ((commit.branches !== parent.branches) &&
           (commit.branches[0] !== parent.branches[0])) {

           if (parent.branches.includes(commit.branches[0])) { //diverging
               if (!graph.reverseArrow || graph.template.arrow.centered) {
                   if (graph.isVertical) {
	...
	class Template {
	    constructor(options) {
	        ...
	        this.unreachableColours = ["#e7e7e7", "#c0c0c0", "#f80000"];
	        if (options.unreachableColours &&
	            (options.unreachableColours.length === 3))
	            this.unreachableColours = options.unreachableColours;
	        // Branch style
	        this.branch = {
	        ...
	        this.arrow = {
	            ...
	            angle: options.arrow.angle || Math.PI / 7,
	            centered: options.arrow.centered || false,
	        };
	        // Commit style
	        this.commit = {
	...
	function templateExtend(selectedTemplate, options) {
	    ...
	    return {
	        ...
	        unreachableColours: options.unreachableColours ||
	                            template.unreachableColours,
	...
	class Commit {
	    constructor(options) {
	        ...
	        this.unreachable = options.unreachable ? options.unreachable : "no";
	        // Set callbacks
	...
	class GitgraphCore {
	    ...
	    createBranch(args) {
	        ...
	        if (options.parentCommitHash)
	            branch$1.startingPoint = options.parentCommitHash;
	        return branch$1;
	    }
	    /**
	    ...
	    computeBranchesColor(commits, branchesPaths) {
	        ...
	        Array.from(branchesPaths).forEach(([branch]) => {
               var tipHash = this.refs.getCommit(branch.name);
               var tipCommit = this.commits.filter(function (value, index, array) {
                   return value.hash === tipHash;
               });
               if (tipCommit[0].unreachable !== "no")
                   branch.computedColor = this.template.unreachableColours[0];
               else branch.computedColor = branch.style.color ||
	...
	var PADDING_X = 10;
	var PADDING_Y = 5;
	function createBranchLabel(branch, commit, detachedColour) {
	    var mainColour = branch.style.label.strokeColor || commit.style.color;
	    if (commit.unreachable === "detached HEAD")
	        mainColour = detachedColour;
	    var rect = createRect({
	        width: 0,
	        height: 0,
	        borderRadius: branch.style.label.borderRadius,
	        stroke: mainColour,
	        strokeWidth: branch.style.label.strokeWidth,
	        fill: utils.contrastingGrey(mainColour, branch.style.label.bgColor),
	    });
	    var textColour = branch.style.label.color || commit.style.color;
	    if (commit.unreachable === "detached HEAD")
	        textColour = detachedColour;
	    var text = createText({
	        content: branch.name,
	        translate: {
	            x: PADDING_X,
	...
	function createTag(tag) {
	    ...
	    var text = createText({
	        content: tag.name,
	        fill: utils.contrastingGrey(tag.style.bgColor, tag.style.color),
	        font: tag.style.font,
	        translate: { x: 0, y: 0 },
	    });
	...
	function createGitgraph(graphContainer, options) {
	    ...
	    function adaptSvgOnUpdate() {
	        ...
	        function positionCommitsElements() {
	            ...
	            var padding = 10;
	            // Ensure commit's elements (branch labels, message etc.) are well positioned.
	            // It can't be done at render time since elements size is dynamic.
	            Object.keys(commitsElements).forEach(function (commitHash) {
	                var _a = commitsElements[commitHash], branchLabels = _a.branchLabels, tags = _a.tags, message = _a.message;
	                ...
	                branchLabels.forEach(function (branchLabel) {
	                    var branchLabelHeight = branchLabel.getBBox().height;
	                    moveElement(branchLabel, x,
	                        (PADDING_Y - branchLabelHeight - 15) / 2);
	                    // NOT SURE WHY '15', BUT LOOKS RIGHT
	    ...
	    function moveElement(target, x, y = 0) {
	        var transform = target.getAttribute("transform") || "translate(0, 0)";
	        var firstpos1 = transform.lastIndexOf(",") + 1;
	        var firstpos2 = transform.lastIndexOf(" ") + 1;
	        if (firstpos1 > firstpos2) firstpos2 = firstpos1;
	        var lastpos = transform.lastIndexOf(")");
	        var oldY = parseInt(transform.substring(firstpos2, lastpos));
	        if (!isNaN(oldY)) y += oldY;
	        target.setAttribute("transform", transform.replace(/translate\(([\d\.]+),? ([\d\.]+)\)/, "translate(" + x + ", " + y + ")"));
	    }
	    ...
	    function renderCommits(commits) {
	        ...
	        function renderArrows(commit) {
	            ...
	            return commit.parents.map(function (parentHash) {
	                ...
	                if (!parent)
	                    return null;
                   var baseColour = parent.style.dot.color;
                   if (commit.branches[0] == parent.branches[0]) {
                       baseColour = commit.style.dot.color;
                   } else if (parent.branches.includes(commit.branches[0])) {
                       baseColour = commit.style.dot.color;
                   }
                   var arrowColour;
                   if (commit.unreachable !== "no")
                       arrowColour = gitgraph.template.unreachableColours[1];
                   else if (gitgraph.template.arrow.color)
                       arrowColour = gitgraph.template.arrow.color;
                   else arrowColour = utils.contrastingGrey(baseColour, "#000000");
	    ...
	    function renderMessage(commit) {
	        if (!commit.style.message.display || (commit.unreachable === "yes")) {
	            return null;
	        }
	        var message;
	        ...
	        return branches.map(function (branch) {
	            ...
	            if (!gitgraph.branchLabelOnEveryCommit) {
	                ...
	                if (commit.hash !== commitHash) {
	                    ...
	                    if (commit.hash !== branch.startingPoint) return null;
	                }
	            }
	            // don't show labels on unreachable branches
	            if (commit.unreachable === "yes") return null;
	            var branchLabel = branch.renderLabel
	                ? branch.renderLabel(branch)
	                : createBranchLabel(branch, commit,
	                                    gitgraph.template.unreachableColours[2]);
	    ...
	    function renderDot(commit) {
	      ...
	      */
	        ...
	        var fillColour = "";
           if (commit.unreachable !== "no")
               fillColour = gitgraph.template.unreachableColours[0];
           else if (commit.style.dot.color) fillColour = commit.style.dot.color;
           var circle = createCircle({
	        ...
	        if (commit.unreachable !== "no") {
	            useCirclePath.setAttribute("stroke",
	                                       gitgraph.template.unreachableColours[1]);
	        } else {
	            useCirclePath.setAttribute("stroke",
	                                       commit.style.dot.strokeColor || "");
	        }
	        ...
	        var dotText = commit.dotText
	            ? createText({
	                ...
	                fill: utils.contrastingGrey(fillColour, "#000000"),
	    ...
	    function setBranchLabelRef(commit, branchLabel) {
	        if (!commitsElements[commit.hashAbbrev]) {
	            initCommitElements(commit);
	        }
	        commitsElements[commit.hashAbbrev].branchLabels.push(branchLabel);
	    }
